...
func renderRange(g *event.Group, repo string) string {
	...
	if title := projectTitle(repo); title != "" {
		b.WriteString(title)
		b.WriteString("\n")
	} else if header := displayHeader(g.Header); header != "" {
	...
	for i, name := range names {
		...
		line := fmt.Sprintf("%s %s %s   %s",
			dimStyle.Render(prefix), staticMark(s.State()), padRight(s.Label(), labelWidth), dimStyle.Render(s.Intent()))
...
func renderEvidence(t *event.Tree) string {
	...
	var lines []string
	if header := displayHeader(t.Header); header != "" {
...
func evidenceLine(prefix, name string, nameWidth, countWidth int, l *event.Leaf) string {
	...
	line := fmt.Sprintf("%s %s %s   %s", dimStyle.Render(prefix), staticMark(l.State()), padRight(name, nameWidth), count)
