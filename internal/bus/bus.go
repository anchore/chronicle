/*
Package bus provides access to a singleton instance of an event bus (provided by the calling application). The event
bus is intended to allow for the library to publish events which library consumers can subscribe to. These events
...
(e.g. function returns and channels) and not depend on bus subscriptions for critical interactions (e.g. one part of
the lib publishes an event and another part of the lib subscribes and reacts to that event). The bus is provided only
as a means for consumers to observe events emitted from the library (such as to provide a rich UI) and not to allow
...
// Get returns the current singleton event bus publisher (may be nil if no bus has been set).
