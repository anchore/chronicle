// Package render is the output layer's presentation of a dependency.Diff: the
...
// grouping — plus the display Config (Mode) that drives them. It lives here in
// the release output tree, beside the encoders that consume it, rather than
// under the core dependency package: it imports dependency for its data but is a
// presentation concern, so the core stays pure data + algorithms. Each encoder
// wraps this text in its own markup.
package render
...
type Config struct {
	...
	// ShowRemaining adds the "remaining" vulnerability rollup — the carried-over
	// vulns still present in the latest scan that this release did not clear
	// (Diff.Remaining). It is opt-in because that set spans the whole latest
	// scan, not just the changed packages, so it can be large and is the current
	// standing burden rather than the release's delta. Meaningful only when the
	// diff was vuln-annotated.
	ShowRemaining bool
}
...
// ShowsRemaining reports whether the remaining vulnerability rollup should be
// rendered. Nil-safe (a nil config does not show it).
func (c *Config) ShowsRemaining() bool {
	return c != nil && c.ShowRemaining
}
