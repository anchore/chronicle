...
func Detect(gitter fileLister, cfg Config, sinceRef, untilRef string) (*release.ToolchainData, error) {
	...
	byTool := make(map[string]Detector, len(detectors))
...
func diffRequirements(since, until map[reqKey]string, byTool map[string]Detector) *release.ToolchainData {
...
func reconcile(updates []release.ToolchainUpdate) []release.ToolchainWarning {
	byTool := make(map[string][]release.ToolchainUpdate)
	...
	for _, tool := range toolOrder {
		...
		warnings = append(warnings, release.ToolchainWarning{
			Tool:    tool,
			Message: fmt.Sprintf("%s sources disagree on the resulting version: %s", release.ToolLabel(tool), strings.Join(parts, "; ")),
			Files:   files,
		})
	}
	return warnings
...
func selectedDetectors(ecosystems []string) []Detector {
	if len(ecosystems) == 0 {
