...
func scan(pkgs ...Package) Scan {
	return Scan{Packages: pkgs}
}
...
func TestCompare(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "empty scans produce empty diff",
			since: scan(),
			until: scan(),
			cmp:   intComparer{},
			want:  nil,
		},
		{
			...
			since: scan(pkg("foo", "1", "go-module")),
			until: scan(),
			cmp:   intComparer{},
			want: []PackageChange{
		...
		},
		{
			...
			since: scan(),
			until: scan(pkg("bar", "2", "go-module")),
			cmp:   intComparer{},
			want: []PackageChange{
		...
		},
		{
			...
			since: scan(pkg("lib", "1", "npm")),
			until: scan(pkg("lib", "2", "npm")),
			cmp:   intComparer{},
			want: []PackageChange{
		...
		},
		{
			...
			since: scan(pkg("lib", "3", "npm")),
			until: scan(pkg("lib", "1", "npm")),
			cmp:   intComparer{},
			want: []PackageChange{
		...
		},
		{
			...
			since: scan(pkg("stable", "5", "go-module")),
			until: scan(pkg("stable", "5", "go-module")),
			cmp:   intComparer{},
			want:  nil,
		},
		{
			name:  "unknown direction (ok=false) classifies as updated",
			since: scan(pkg("lib", "v1.0.0", "go-module")),
			until: scan(pkg("lib", "v2.0.0", "go-module")),
