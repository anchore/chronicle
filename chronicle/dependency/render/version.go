...
// VersionTransition renders a change's version movement for display: shortened
// "from → to" for updates/downgrades, or a single shortened version for
// additions (the new version) and removals (the version that left). Versions
// render as plain text; use VersionTransitionWith to style each token.
func VersionTransition(c dependency.PackageChange) string {
	return VersionTransitionWith(c, nil)
}

// VersionTransitionWith is VersionTransition with each (shortened) version token
// rendered through code, so an encoder can wrap the versions in inline-code
// markup while the → arrow stays outside it (e.g. `v1.0.1` → `v1.0.2` rather
// than `v1.0.1 → v1.0.2`). A nil code yields bare version text.
...
// VulnNote summarizes a change's vulnerability effect as a short phrase (no
...
// was not annotated). IDs render as plain text; use VulnNoteWith to link them.
func VulnNote(c dependency.PackageChange) string {
	return VulnNoteWith(c, nil)
}

// VulnNoteWith is VulnNote with each vulnerability ID rendered through link, so
// encoders can hyperlink CVE/GHSA IDs to their data source. A nil link yields
// bare IDs (identical to VulnNote).
